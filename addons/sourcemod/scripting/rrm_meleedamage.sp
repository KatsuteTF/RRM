...
{
	name = "[RRM] Melee Damage Modifier",
	author = RRM_AUTHOR,
	description = "Modifier that changes damage done from your melee weapon.",
	version = RRM_VERSION,
	url = RRM_URL
};
...
{
	cMin = CreateConVar("rrm_meleedamage_min", "2.0", "Minimum value for the random number generator.");
	cMax = CreateConVar("rrm_meleedamage_max", "3.5", "Maximum value for the random number generator.");
	...
	AutoExecConfig(true, "rrm_meleedamage", "rrm");
}
...
{
	RRM_Register("Melee damage", gMin, gMax, false, RRM_Callback_Secondary);
}
